...
List available models. Picking one writes `agent.<active>.model` to the project's `opencode.json[c]` so the choice is shared with teammates — see [Per project](/docs/config#per-project) for the full persistence semantics and the `OPENCODE_DISABLE_PROJECT_CONFIG` opt-out.
