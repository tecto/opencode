...
## System prompt budget (BP-002)

The first-message system prompt was reduced from ~4,549 to ~2,017 tokens (delta ≈ 2,531 cl100k tokens) by trimming the global `~/.config/opencode/AGENTS.md` (kept 4 high-bite behavioral rules; removed Crux-framework + cruxdev-routing sections), dropping the verbose-XML `<location>` field from skill emission, dropping the redundant skills preamble, and shortening the `Instructions from:` prefix to a scope-aware label (`# ~/<rel-path>` for global, `# <worktree-relative-path> (project)` for project files — disambiguates root vs per-package AGENTS.md in monorepos). See `build_plans/BUILD_PLAN_002_TRIM_GLOBAL_AGENTS_AND_SKILLS_XML.md` for the audit trail. Project AGENTS.md content is unchanged; project-specific rules still apply.
